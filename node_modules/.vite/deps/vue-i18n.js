...
var inBrowser, mark, measure, RE_ARGS, makeSymbol, generateFormatCacheKey, friendlyJSONstringify, isNumber, isDate, isRegExp, isEmptyObject, assign, _globalThis, getGlobalThis, hasOwnProperty, isArray2, isFunction, isString2, isBoolean, isSymbol2, isObject2, isPromise, objectToString, toTypeString, isPlainObject, toDisplayString, RANGE, hasWarned, isNotObjectOrIsArray;
var init_shared_esm_browser = __esm({
  "node_modules/@intlify/shared/dist/shared.esm-browser.js"() {
    ...
    RANGE = 2;
    hasWarned = {};
    isNotObjectOrIsArray = (val) => !isObject2(val) || isArray2(val);
  }
});
...
function resolveWithKeyValue(obj, path) {
  return isObject3(obj) ? obj[path] : null;
}
...
function createMessageContext(options = {}) {
  ...
  const pluralRule = isObject3(options.pluralRules) && isString3(locale) && isFunction2(options.pluralRules[locale]) ? options.pluralRules[locale] : pluralDefault;
  const orgPluralRule = isObject3(options.pluralRules) && isString3(locale) && isFunction2(options.pluralRules[locale]) ? pluralDefault : void 0;
  ...
  function message(key) {
    const msg = isFunction2(options.messages) ? options.messages(key) : isObject3(options.messages) ? options.messages[key] : false;
  ...
  const type = isPlainObject2(options.processor) && isString3(options.processor.type) ? options.processor.type : DEFAULT_MESSAGE_DATA_TYPE;
...
function fallbackWithSimple(ctx, fallback, start) {
  return [.../* @__PURE__ */ new Set([
    start,
    ...isArray3(fallback) ? fallback : isObject3(fallback) ? Object.keys(fallback) : isString3(fallback) ? [fallback] : [start]
  ])];
}
function fallbackWithLocaleChain(ctx, fallback, start) {
  ...
  if (!chain) {
    ...
    const defaults = isArray3(fallback) || !isPlainObject2(fallback) ? fallback : fallback["default"] ? fallback["default"] : null;
...
function appendItemToChain(chain, target, blocks) {
  ...
  if (!chain.includes(target)) {
    ...
    if (target) {
      ...
      chain.push(locale);
      if ((isArray3(blocks) || isPlainObject2(blocks)) && blocks[locale]) {
...
function getDefaultLinkedModifiers() {
  return {
    upper: (val, type) => {
      return type === "text" && isString3(val) ? val.toUpperCase() : type === "vnode" && isObject3(val) && "__v_isVNode" in val ? val.children.toUpperCase() : val;
    },
    lower: (val, type) => {
      return type === "text" && isString3(val) ? val.toLowerCase() : type === "vnode" && isObject3(val) && "__v_isVNode" in val ? val.children.toLowerCase() : val;
    },
    capitalize: (val, type) => {
      return type === "text" && isString3(val) ? capitalize2(val) : type === "vnode" && isObject3(val) && "__v_isVNode" in val ? capitalize2(val.children) : val;
    }
  };
}
...
function createCoreContext(options = {}) {
  ...
  const version = isString3(options.version) ? options.version : VERSION;
  ...
  const fallbackLocale = isArray3(options.fallbackLocale) || isPlainObject2(options.fallbackLocale) || isString3(options.fallbackLocale) || options.fallbackLocale === false ? options.fallbackLocale : _locale;
  ...
  const fallbackContext = isObject3(options.fallbackContext) ? options.fallbackContext : void 0;
  const internalOptions = options;
  const __datetimeFormatters = isObject3(internalOptions.__datetimeFormatters) ? internalOptions.__datetimeFormatters : /* @__PURE__ */ new Map();
  const __numberFormatters = isObject3(internalOptions.__numberFormatters) ? internalOptions.__numberFormatters : /* @__PURE__ */ new Map();
  const __meta = isObject3(internalOptions.__meta) ? internalOptions.__meta : {};
  _cid++;
  const context = {
    version,
    cid: _cid,
...
function translate(context, ...args) {
  ...
  const defaultMsgOrKey = isString3(options.default) || isBoolean2(options.default) ? !isBoolean2(options.default) ? options.default : !messageCompiler ? () => key : key : fallbackFormat ? !messageCompiler ? () => key : key : "";
  ...
  if (!resolvedMessage && !(isString3(format3) || isMessageAST(format3) || isMessageFunction(format3))) {
    if (enableDefaultMsg) {
      format3 = defaultMsgOrKey;
      cacheBaseKey = format3;
    }
  }
  if (!resolvedMessage && (!(isString3(format3) || isMessageAST(format3) || isMessageFunction(format3)) || !isString3(targetLocale))) {
    return unresolving ? NOT_REOSLVED : key;
  }
  if (isString3(format3) && context.messageCompiler == null) {
  ...
  const ret = postTranslation ? postTranslation(messaged, key) : messaged;
  {
    const payloads = {
      timestamp: Date.now(),
      key: isString3(key) ? key : isMessageFunction(format3) ? format3.key : "",
      locale: targetLocale || (isMessageFunction(format3) ? format3.locale : ""),
      format: isString3(format3) ? format3 : isMessageFunction(format3) ? format3.source : "",
      message: ret
    };
...
function escapeParams(options) {
  if (isArray3(options.list)) {
    options.list = options.list.map((item) => isString3(item) ? escapeHtml2(item) : item);
  } else if (isObject3(options.named)) {
...
function resolveMessageFormat(context, key, locale, fallbackLocale, fallbackWarn, missingWarn) {
  ...
  for (let i = 0; i < locales.length; i++) {
    ...
    if (isString3(format3) || isMessageAST(format3) || isMessageFunction(format3)) {
      break;
    }
  ...
  return {
    ...
    onError: (err) => {
      onError && onError(err);
      {
        ...
        const codeFrame = err.location && _source && generateCodeFrame3(_source, err.location.start.offset, err.location.end.offset);
        const emitter = context.__v_emitter;
        if (emitter && _source) {
...
function datetime(context, ...args) {
  ...
  if (!isPlainObject2(format3) || !isString3(targetLocale)) {
    return unresolving ? NOT_REOSLVED : key;
  }
...
var init_core_base_esm_browser = __esm({
  "node_modules/@intlify/core-base/dist/core-base.esm-browser.js"() {
    ...
    toDisplayString2 = (val) => {
      return val == null ? "" : isArray3(val) || isPlainObject2(val) && val.toString === objectToString2 ? JSON.stringify(val, null, 2) : String(val);
    };
    RANGE2 = 2;
    hasWarned2 = {};
    CompileWarnCodes = {
    ...
    capitalize2 = (str) => `${str.charAt(0).toLocaleUpperCase()}${str.substr(1)}`;
    ...
    compileCache = /* @__PURE__ */ Object.create(null);
    isMessageAST = (val) => isObject3(val) && (val.t === 0 || val.type === 0) && ("b" in val || "body" in val);
